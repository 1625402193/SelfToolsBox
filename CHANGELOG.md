...
### 新增（批量复制图片）
- **目标路径支持临时停用开关**：每个目标路径行新增「启用」开关，临时停用后该路径保留配置但不参与索引与复制，无需删除后重新选择，重新打开开关即可恢复。
  - 停用的路径整行灰显，开关状态随配置持久化，重启软件后保持。
  - 切换开关立即同步主进程索引缓存与界面的「目标路径目录分布」，不会沿用被停用路径的旧索引。
  - 「重建目标索引」「匹配」只处理已启用的目标路径；全部停用时执行会给出明确提示并中止。

## v1.1.5 — 2026-09-14

### 新增
- **「批量复制图片」匹配规则重做为四级优先级回退**，参考成熟脚本的实践大幅提升自动命中率。对每一张源图在每个目标路径内独立按序尝试，命中即停：
  1. **同名精确匹配**：目标中已存在同名文件 → 直接定位该文件所在目录（替换场景），大小写不敏感。
  2. **后缀词匹配**：文件名以后缀词结尾时（默认 `Fang` / `Yuan`，可自定义），定位「同前缀且同后缀」的资源所在目录，避免 `Fang` 与 `Yuan` 互相混淆。
  3. **前缀逐级回退**：按 `_` 分段，从最长前缀逐级缩短，命中第一个存在已有资源的前缀，即最精确的那一级（替代原先固定分类粒度一刀切的做法）。
  4. **尺寸兜底**：以上均未命中时，按「宽×高」索引兜底；读不出尺寸的图片不参与。
  - 四条规则均可单独开关；新增「后缀词」「最短前缀分段」配置项。
- **多候选目录改为优先自动消歧，减少人工干预**：任一级规则命中多个目录时，先用源图分辨率消歧（只保留含同分辨率资源的目录）；仍无法确定时可按「命中数量最多」自动决定（可关闭）；最后才弹窗询问。弹窗中会标注各候选目录已有资源的分辨率，便于判断（如头像类的 `TouXiang` 192×192 与 `TouXiangLOD2` 48×48）。
- **同名文件增加内容比对**：目标已存在同名文件时，先按「文件大小 + MD5」判断内容是否一致，完全相同则自动跳过，不复制也不询问，大幅减少无效的覆盖确认。可通过开关关闭。
- 索引结构相应调整：统计与目录分布、同名/前缀/后缀词查询表、尺寸索引分三个文件存放（仍是每个目标路径各一份，互不干扰）。
- 界面与日志区分展示匹配方式：匹配结果按「同名 / 后缀词 / 前缀 / 尺寸」统计命中分布并标注由分辨率自动消歧的数量；操作日志逐条注明每张图经由哪条规则命中；执行结果区分「新增 / 替换 / 内容相同跳过 / 用户跳过 / 失败」。

### 移除
- **移除「分类粒度」选项及其背后的分类键机制**：前缀逐级回退已能自动落到最精确的那一级，不再需要手动指定取几段，也避免了粒度设置不当导致的误归类。
  - 「源路径分类结果」改为「源路径待处理图片」，平铺展示每张图的文件名、尺寸与来源路径（不再按分类键分组）。
  - 「目标索引明细」改为「目标路径目录分布」，按目录展示图片数量与分辨率分布（可点击目录名直接打开），比原先罗列数千个分类键更直观，也便于核对分辨率消歧的依据。

> 升级提示：索引结构已变更，本版本首次使用时请先执行一次「重建目标索引」。

### 修复（媒体评分）
- **修复大文件夹（几千张图片，尤其混杂 RAW 如 jpg+arw）预览时应用崩溃的问题**：
  - 左侧缩略图列表改为虚拟滚动，只挂载可视区域附近的行，不再一次性渲染成千上万个 `<img>`。
  - 主进程新增 RAW 预览提取的并发限制（同时最多 3 个），超出的请求排队等待，避免瞬间打开大量 RAW 文件耗尽内存/CPU。

### 新增（媒体评分）
- **预览类型筛选**：扫描改为两步——先快速探测文件夹下各扩展名的文件数量（不做 stat，很快），展示类型勾选面板，用户确认要预览的类型后再真正加载；未选中的类型完全不参与后续扫描、渲染和缩略图生成（不仅是界面隐藏）。这也是修复上述崩溃问题的关键一环：可以直接把重型的 RAW 类型排除在预览之外。
- **导出/删除时识别同名不同类型文件**：例如同一文件夹下的 `5099.jpg` 与 `5099.arw`。按评分导出或删除时，若检测到已选中的文件存在同名不同类型的"兄弟文件"（哪怕它的类型未被选入预览），会弹窗询问：
  - 「仅导出/删除预览类型」：保持原有行为，只处理已选中类型的文件。
  - 「两种类型都导出/删除」：导出时会在目标文件夹下按类型（如 `JPG` / `ARW`）分别创建子文件夹存放；删除时两种类型一起移入回收站。
- **新增「按评分删除」功能**：评分完成后，可勾选一个或多个星级（含未评分），一键将匹配的文件移到回收站；同样支持"同名不同类型文件是否一起删除"的询问，删除前有二次确认。
